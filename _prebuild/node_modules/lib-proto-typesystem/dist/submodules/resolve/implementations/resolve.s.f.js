...
function mapOptional($, a) {
    return pl.optional($, ($) => [true, a($)], () => [false]);
}
...
function resolve($d, $se) {
    ...
    const Local__Namespace = ($, $p) => {
        ...
        const v_namespaces = $d.resolveDictionary($.namespaces.dictionary, {
            'map': ($, $l) => {
                return pl.cc($.value, ($) => {
                    switch ($[0]) {
            ...
            case 'type reference': return pl.ss($, ($) => ['type reference', pl.cc($, ($) => {
                    switch ($[0]) {
                        ...
                        case 'external': return pl.ss($, ($) => {
                            ...
                            return ['external', {
                                    'namespaces': v_namespaces,
                                    'type': getAnnotatedEntry(selectLocalNSFromNS2(selectNS2FromSelection(v_namespaces)).types, $.type),
                                }];
                        });
    ...
    const Namespace__Selection = ($, $p) => {
        ...
        return {
            ...
            'arguments': $.arguments.dictionary.__mapWithKey(($, key) => {
                return {
                    'constraints': {
                        'parameter': getEntry(selectLocalNSFromNS2(v_namespace.referent).parameters.local, key, $.annotation)
                    },
                    'content': {
